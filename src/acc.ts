...
export function calculateAccuracy(mode: OsuMode, score: Score): number {
    switch (mode) {
        ...
        case "taiko": return calcTaikoAccuracy(score);
        default: return 0;
    }
}

export function calcTaikoAccuracy(score: Score): number {
    const { "0": n0, "100": n100, "300": n300 } = score;

    const total = n0 + n100 + n300;
    if (total === 0) return 0;

    const acc = (n100 * 0.5 + n300) / total;
